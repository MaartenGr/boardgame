...
def compare_players(df, player_list):
    ...
    sidebar_graph(df, player_one, player_two)
...
def sidebar_graph(df, player_one, player_two):
    to_plot = df.loc[(df[player_one + "_played"] == 1)&
                     (df[player_two + "_played"] == 1), :]
    to_plot = to_plot.sort_values("Date").set_index("Date").resample("3D").count().reset_index()
